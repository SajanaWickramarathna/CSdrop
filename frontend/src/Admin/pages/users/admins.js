...
export default function Admins({ onAdminSelect, onAdminDelete }) {
  ...
  const getStatusChip = (status) => {
    switch(status) {
      ...
      case 'super-admin': return <Chip label="Super Admin" color="primary" size="small" />;
      ...
      <Box className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        ...
        <Box className="flex items-center gap-2 w-full md:w-auto">
          ...
          <Tooltip title="Refresh">
            <IconButton onClick={fetchAdmins} color="primary"><Refresh /></IconButton>
          </Tooltip>
        </Box>
      </Box>
                ...
                <TableRow key={admin.user_id} hover>
                  ...
                  <TableCell>
                    <Box className="flex items-center gap-2">
                      <Avatar src={admin.profilePic && `http://localhost:3001${admin.profilePic}`} sx={{ width: 36, height: 36 }}>
                  ...
                  <TableCell align="center">
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
                      <Tooltip title="Edit">
                        <IconButton color="primary" onClick={() => onAdminSelect({ id: admin.user_id, email: admin.email })}>
                          <Edit />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Delete">
                        <IconButton color="error" onClick={() => onAdminDelete({ id: admin.user_id, email: admin.email })}>
                          <Delete />
                        </IconButton>
                      </Tooltip>
                    </Box>
